...
function toResultCardClipFromCaption(item, mediaUrl) {
    ...
    return {
        ...
        thumbnail_url: item?.caption_preview_url || item?.caption_thumbnail_url || "",
        preview_image_url: item?.caption_preview_url || item?.caption_thumbnail_url || "",
...
export default function CaptionsPage({ projectId = "", autoOpenFirst = false }) {
    ...
    useEffect(() => {
        ...
        async function loadCaptions() {
            ...
            try {
                ...
                if (!response.ok) {
                    if (!cancelled) {
                        setError(data?.detail || "Unable to load subtitles");
                        setItems([]);
                    }
                    return;
                }
                if (cancelled) return;
                const rawItems = projectId ? data.captions : data.items;
                const nextItems = Array.isArray(rawItems) ? rawItems : [];
                const filteredItems = projectId
                    ? nextItems.filter((item) => {
                        const merged = `${item.caption_title || ""} ${item.caption_description || ""}`.toLowerCase();
                        const matchesQuery = !query || merged.includes(query.toLowerCase());
                        const matchesStatus = !status || (item.caption_status || CAPTION_STATUS_FALLBACK) === status;
    ...
    useEffect(() => {
        if (!projectId || !autoOpenFirst || didAutoOpen) return;
        if (!projectMeta || projectMeta.status !== "completed") return;
        if (!items.length) return;
        handlePreview(items[0]);
        setDidAutoOpen(true);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [projectId, autoOpenFirst, didAutoOpen, projectMeta, items]);
    ...
    return (
        <div className="captions-page-shell flex-1 overflow-y-auto overflow-x-hidden md:overflow-x-visible px-4 py-6 md:p-8 space-y-5 md:space-y-6">
            ...
            <section className="rounded-2xl border border-slate-300 dark:border-white/10 bg-white/5 p-3 md:p-5 space-y-4">
                {!canCreateCaption ? (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-md">
                    <div className="flex w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-slate-300 dark:border-white/10 bg-white dark:bg-zinc-950 shadow-2xl">
                        <div className="flex items-center justify-between border-b border-slate-300 dark:border-white/10 px-4 py-3">
                            <div>
                                <p className="text-sm font-semibold text-slate-900 dark:text-white">{previewItem.caption_title || t("reels.previewTitle", "Reel preview")}</p>
