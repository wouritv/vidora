...
export default function ReelsProjectsPage() {
    ...
    const handleOpenProject = (project) => {
        ...
        // Reuse process tracking UI bound to this project for non-completed states.
        if (project?.status === "processing" || project?.status === "failed" || project?.status === "cancelled") {
            navigate(`/dashboard/reel-generator?project_id=${encodeURIComponent(projectId)}`);
            return;
        }

        // Defensive fallback: never open raw creation view when project context exists.
        navigate(`/dashboard/reel-generator?project_id=${encodeURIComponent(projectId)}`);
    };
                    ...
                    {!loading && items.map((item) => (
                        ...
                        >
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-zinc-400">
                                <span>{t("projects.tableOutputs", "Reels")}: {Number(item.output_count || 0)}</span>
                                <span>•</span>
                    ...
                    <table className="min-w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-300 dark:border-white/10 text-left text-slate-500 dark:text-zinc-400 text-xs md:text-sm">
                                ...
                                <th className="hidden sm:table-cell px-2 md:px-3 py-3 font-medium">{t("projects.tableOutputs", "Reels")}</th>
                                <th className="hidden md:table-cell px-2 md:px-3 py-3 font-medium">{t("generatedMedia.tableDuration", "Duration")}</th>
                        ...
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan={7} className="px-3 py-10 text-center text-slate-500 dark:text-zinc-400">
                                        <span className="inline-flex items-center gap-2"><Loader2 size={14} className="animate-spin" /> {t("reels.loading", "Loading...")}</span>
                                    </td>
                                </tr>
                            ) : null}
                            ...
                            {!loading && items.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="px-3 py-10 text-center text-slate-500 dark:text-zinc-400">{t("common.noItemsFound", "Aucun element trouve")}</td>
                                </tr>
                            ) : null}
                            ...
                            {!loading && items.map((item) => (
                                ...
                                >
                                    ...
                                    <td className="hidden sm:table-cell px-2 md:px-3 py-2 md:py-3 text-slate-700 dark:text-zinc-300">{Number(item.output_count || 0)}</td>
                                    <td className="hidden md:table-cell px-2 md:px-3 py-2 md:py-3 text-slate-700 dark:text-zinc-300">{formatDurationHms(item.source_duration)}</td>
                                    <td className="px-2 md:px-3 py-2 md:py-3">
                                        <span className={`inline-flex rounded-full border px-2 py-1 text-xs ${statusClass(item.status)}`}>{statusLabel(item.status)}</span>
                                    </td>
