...
// Same list/rename/delete pattern as ReelsProjectsPage / CaptionProjectsPage:
// anonymous stories are projects like any other module, listed via
// /api/projects?project_type=anonymous_story and opened either into the
// creation/progress flow (still running) or the editor (done).
...
export default function AnonymousStoriesProjectsPage() {
    const { user } = useAuth();
    ...
    const handleOpenProject = (project) => {
        ...
        if (project?.status === "completed") {
            navigate(`/dashboard/anonymous-stories/projects/${projectId}`);
            return;
        }

        // processing, failed, cancelled -- and any unexpected value --
        // all resume into the creation/progress page, same as reels/captions.
        navigate(`/dashboard/anonymous-stories/new?project_id=${encodeURIComponent(projectId)}`);
    };
    ...
    return (
        <div className="captions-page-shell flex-1 overflow-y-auto p-8 space-y-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 className="text-3xl font-black tracking-tight">{t("projects.anonymousStoriesTitle", "Mes projets Histoires anonymes")}</h1>
